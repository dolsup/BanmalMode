...
function haeche(parsedString) {
    ...
    for(var i = 0; i < parsed.length; i++) {
        ...
        if(isInflect(parsed[i])) { // 활용형 여부
            ...
            //// 조건 검사 1 : 해요체 => 해체
            if(isSame(parsed[i], replace.hae.fromYo.Inflect)) { // 활용형 자체가 사전에 정의되었나
                newstr = replace.hae.fromYo.Inflect[styMor(parsed[i])][0];
                flag = true; // 교체됨
            } else { // 정의되지 않았다면 활용형을 분해해서 각 형태소를 조건 검사
                bricks = breakInflect(parsed[i]);
                for(var b = 0; b < bricks.length; b++) {
                    if(!config.si && bricks[b].toString() === si.toString()) bricks = bricks.splice(b-1, 1); // 선어말어미 -시-의 삭제
                    if(isSame(bricks[b], replace.hae.fromYo)) { // 분해된 각 형태소를 사전 정의된 패턴으로 검사
                        bricks[b] = replace.hae.fromYo[styMor(bricks[b])];
                        flag = true; // 교체 가능
                    }
                }
            }
            ...
            if(flag) { // 교체가 일어났을 때
                if(bricks) { // 활용형을 분해했을 때
                    if(shorten[normalizeMorphemes(bricks)]) // 정규화된 형태소 조각들의 축약 패턴이 정의되어 있다면
                        ...
                        if(!newvowel) newstr = assembly(bricks); // 모음 축약이 없으면 그냥 강제로 붙임. (예외)
                    }
                }
                //// 전체 문장에서 바뀐 문자열로 교체함.
        ...
        //// 활용형이 아님
        if(!config.si && parsed[i].toString() === si.toString()) { // 선어말어미 -시- 제거
            output = output.slice(0, originpos) + output.slice(originpos+originlen);
            extra -= originlen;
        }
        if(isSame(parsed[i], replace.hae.fromYo)) { // 해요체 => 해체
            output = output.slice(0, originpos) + replace.hae.fromYo[styMor(parsed[i])][0] + output.slice(originpos+originlen);
            extra += replace.hae.fromYo[styMor(parsed[i])][0].length - originlen;
        }
        //// 활용형이 아님
    }
}
