...
지금은 다른 존댓말들 => 해체를 기본으로 하고 있지만, 나중에는 여러 문체들이 중간 문체(아마도 해체가 될 듯)를 거쳐서 모두 상호 변환이 가능하도록 할 예정이지.
...
```
이 글만 보면 완벽하지만, 아직 정의된 패턴이 부족해서 추가해야 돼. 사전식 치환으로 다 하려는 건 아니고, 국어 문법에 따른 변형들(예컨대 모음 축약)을 코드로 정의해야 해.
